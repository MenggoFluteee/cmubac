...
@section('content')
                    ...
                    <div class="modal-body mx-3">
                        ...
                        <div class="row mb-3">
                            <div class="col-12 mb-3">
                                <label class="form-label">Item Account Code</label>
                                <select name="formAddItemAccountCode" id="formAddItemAccountCode" class="form-select">
                                    <option value="">Select Account Code</option>
                                    @foreach ($itemAccountCodes as $code)
                                        <option value="{{ $code->id }}">{{ $code->account_name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    data.forEach(function(item) {
                        const itemName = encodeURIComponent(item.item_name || '');
                        const itemCode = encodeURIComponent(item.item_code || '');
                        const itemDesc = encodeURIComponent(item.item_description || '');
                        const itemUnitOfMeasure = encodeURIComponent(item.item_unit_of_measure || '');
                        const currentPrice = encodeURIComponent(item.current_price || '');
                        const itemCategoryId = item.item_category_id || 0;
                        const accountCodeId = item.account_code_id ||
                            0; // ✅ Ensure account_code_id is captured
                    ...
                    onclick="editItem(
                        ...
                        ${item.is_psdbm}, 
                        '${currentPrice}',
                        ${accountCodeId} // ✅ Pass account code ID correctly
                    )" title="Edit">
                    <i class="fas fa-edit"></i>
                </button>`;

                        var deleteButton = `<button type="button" class="btn btn-sm btn-danger" 
                    onclick="deleteItem(${item.id}, '${itemName}')" title="Delete">
    ...
    <script>
        ...
        // Update your editItem function to decode the URL encoded strings
        function editItem(id, name, code, item_category_id, unit_of_measure, description, is_available, is_psdbm, price,
            ...
            unit_of_measure = decodeURIComponent(unit_of_measure);
            ...
            // ✅ Set Account Code correctly
            $('#formEditItemAccountCode').val(accountId); // Ensures Select2 refreshes properly
